...
STAFF_MALES = ['نزار', 'مايك', 'Ahmed', 'سابا', 'اندريس', 'وليد']
# تم تعديل اسم أحمد ليكون متوافقاً مع القائمة
        ...
        {% if session.get('user') %}
            ...
            {% if session.get('user') == 'خضر' %}
                <div class="card" style="border-right: 5px solid #27ae60;">
                    ...
                    <p>يمكنك الاطلاع على كافة الجداول وتحركات المراسلات، وإرسال رسائل فردية أو جماعية.</p>
                    
                    <form method="POST" action="/send_message">
                        <label>إرسال رسالة إلى:</label>
                        <select name="receiver">
                            <option value="الكل">جميع المعلمين والمربين</option>
            ...
            {% if session.get('user') != 'خضر' %}
                <div class="card" style="margin-top: 20px;">
                    ...
                    <table>
                        ...
                        {% for m in messages %}
                            {% if m.receiver == session.get('user') or m.receiver == 'الكل' %}
                            <tr>
                                <td>{{ m.sender }}</td>
                                <td>{{ m.text }}</td>
                            </tr>
...
def send_message():
    if session.get('user') == 'خضر':
        ...
        MESSAGES.insert(0, {'sender': 'المدير (خضر)', 'receiver': receiver, 'text': text})
    return redirect(url_for('index'))
